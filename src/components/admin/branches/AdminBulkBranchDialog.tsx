...
import { Upload, Download, AlertCircle, CheckCircle, FileUp, Loader2 } from 'lucide-react';
...
export function AdminBulkBranchDialog() {
  ...
  const createBranchesMutation = useMutation({
    mutationFn: async ({ userId, branches }: { userId: string; branches: BranchFormData[] }) => {
      const branchRecords = branches.map(branch => {
        const coords = extractCoordsFromLink(branch.google_maps_link);
        return {
          user_id: userId,
          name: branch.name,
      ...
      const { error } = await supabase.from('branches').insert(branchRecords);
      if (error) throw error;
      return branchRecords.length;
    },
    onSuccess: (count) => {
      toast.success(`${count} branches created successfully`);
  ...
  const parseCSV = (content: string): { valid: BranchFormData[]; errors: string[] } => {
    ...
    dataLines.forEach((line, index) => {
      ...
      if (parts.length < 4) {
        errors.push(`Line ${lineNumber}: Expected at least 4 fields, got ${parts.length}.`);
        return;
      }
      ...
      if (!cityMatch) {
        ...
        errors.push(`Line ${lineNumber}: Invalid city "${city}". Valid: ${validCities}.`);
        return;
      }
  ...
  const handleSubmit = async () => {
    if (!parseResult || parseResult.valid.length === 0 || !clientUserId) return;
    createBranchesMutation.mutate({ userId: clientUserId, branches: parseResult.valid });
  };
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Bulk Add Branches for Client
          </DialogTitle>
          <DialogDescription>
            Upload a CSV file to add multiple branches for a client. All branches will be automatically verified.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              {parseResult.errors.length > 0 && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    <p className="font-medium mb-1">{parseResult.errors.length} error(s):</p>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={createBranchesMutation.isPending || !clientUserId || !parseResult || parseResult.valid.length === 0}
          >
            {createBranchesMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
